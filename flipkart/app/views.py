...
def index(req):
    ...
    print(allproducts)
    print(allcategories)
    return render(req,"index.html",{'allproducts':allproducts ,'allcategories': allcategories})
...
def addtowishlist(req,productid):
    if req.user.is_authenticated:
        ...
        if not Wishlist.objects.filter(userid=userid,productid=productid).exists():
            ...
            messages.info(req,'product is alreday in wishlist')
...
def removewishlist(req, productid):
    if req.user.is_authenticated:
        ...
        messages.error(req,"You need to log in to remove product from your wishlist")
        return redirect("signin")
...
def updateqty(req,qv,productid):
    ...
    if qv==1:
        if cart_item.qty<product.quantity_available:
            ...
            messages.error(req,"only limited stock avaliable")
    else:
        if cart_item.qty>1:
            cart_item.qty-=1
            cart_item.save()
...
def removefromcart(req, productid):
    if req.user.is_authenticated:
        ...
        messages.error(req,"You need to log in to remove product from your cart")
        return redirect("signin")
...
def addtocart(req,productid):
    if req.user.is_authenticated:
        ...
        if new_qty > product.quantity_available:
            messages.error(req,'Cannnot add more items. Only limited stock available')
            return redirect('showcarts')
...
def addprofile(req):
    ...
    if req.method=='POST':
        ...
        if dob:
            ...
            if age<18:
                messages.error(req,"you must be at least 18 your old to create profile")
...
# def addaddress(req):
#     if req.method=='POST':
#         form=AddressForm(req.POST,req.FILES)
#         if form.is_valid():
...
def deleteaddress(req,addressid):
    profile=get_object_or_404(Address,id=addressid)
    profile.delete()
    return redirect('myprofile') 
